...
func (vcd *TestVCD) Test_UpdateAlbSettings(check *C) {
	...
	disabledSettings, err := edge.UpdateAlbSettings(ctx, albSettingsConfig)
	check.Assert(err, IsNil)
	...
	enabledSettingsCustomServiceDefinition, err := edge.UpdateAlbSettings(ctx, albSettingsConfig)
	check.Assert(err, IsNil)
	...
	if vcd.client.Client.APIVCDMaxVersionIs(ctx, ">= 37.0") {
		...
		enabledSettingsIpv6ServiceDefinition, err := edge.UpdateAlbSettings(ctx, albSettingsConfig)
		check.Assert(err, IsNil)
	...
	if vcd.client.Client.APIVCDMaxVersionIs(ctx, ">= 37.1") {
		...
		enabledSettingsTransparentMode, err := edge.UpdateAlbSettings(ctx, albSettingsConfig)
		check.Assert(err, IsNil)
