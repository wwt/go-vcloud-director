...
func (vcd *TestVCD) Test_AnyTypeEdgeGateway(check *C) {
	...
	nsxtEdge, err := adminOrg.GetNsxtEdgeGatewayByName(ctx, vcd.config.VCD.Nsxt.EdgeGateway)
	check.Assert(err, IsNil)
	check.Assert(nsxtEdge, NotNil)

	nsxvEdge, err := vcd.vdc.GetEdgeGatewayByName(ctx, vcd.config.VCD.EdgeGateway, false)
	check.Assert(err, IsNil)
	...
	nsxtAnyTypeEdgeGateway, err := adminOrg.GetAnyTypeEdgeGatewayById(ctx, nsxtEdge.EdgeGateway.ID)
	check.Assert(err, IsNil)
	...
	nsxvAnyTypeEdgeGateway, err := adminOrg.GetAnyTypeEdgeGatewayById(ctx, nsxvEdge.EdgeGateway.ID)
	check.Assert(err, IsNil)
	...
	// common type AnyTypeEdgeGateway
	nsxtOrgAnyTypeEdgeGateway, err := org.GetAnyTypeEdgeGatewayById(ctx, nsxtEdge.EdgeGateway.ID)
	check.Assert(err, IsNil)
	...
	// Structures for NSX-V Edge Gateway differ (because it uses XML API) therefore all fields
	// cannot be compared
	nsxvOrgAnyTypeEdgeGateway, err := org.GetAnyTypeEdgeGatewayById(ctx, nsxvEdge.EdgeGateway.ID)
	check.Assert(err, IsNil)
