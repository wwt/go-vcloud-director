...
func publishCatalog(ctx context.Context, client *Client, catalogUrl string, tenantContext *TenantContext, publishCatalog types.PublishCatalogParams) error {
	...
	err := client.ExecuteRequestWithoutResponse(ctx, catalogUrl, http.MethodPost,
...
func (cat *Catalog) IsSharedReadOnly(ctx context.Context) (bool, error) {
	accessControl, err := cat.GetAccessControl(ctx, true)
	if err != nil {
		return false, err
	}
...
// IsSharedReadOnly returns the state of the catalog read-only sharing to all organizations
func (cat *AdminCatalog) IsSharedReadOnly(ctx context.Context) (bool, error) {
	accessControl, err := cat.GetAccessControl(ctx, true)
	if err != nil {
		return false, err
	}
...
func (cat *Catalog) publish(ctx context.Context, isPublished bool) error {
	if cat.Catalog == nil {
	...
	err = publishCatalog(ctx, cat.client, catalogUrl, tenantContext, publishParameters)
	if err != nil {
		return err
	}

	return cat.Refresh(ctx)
}

// publish publishes a catalog read-only access control to all organizations.
// This operation is usually the second step for a read-only sharing to all Orgs
func (cat *AdminCatalog) publish(ctx context.Context, isPublished bool) error {
...
func (cat *Catalog) SetReadOnlyAccessControl(ctx context.Context, isPublished bool) error {
	if cat.Catalog == nil {
		return fmt.Errorf("cannot set access control, Object is empty")
	}
	err := cat.SetAccessControl(ctx, &types.ControlAccessParams{
	...
	return cat.publish(ctx, isPublished)
}

// SetReadOnlyAccessControl will create or rescind the read-only AdminCatalog sharing to all organizations
func (cat *AdminCatalog) SetReadOnlyAccessControl(ctx context.Context, isPublished bool) error {
	if cat.AdminCatalog == nil {
		return fmt.Errorf("cannot set access control, Object is empty")
	}
	err := cat.SetAccessControl(ctx, &types.ControlAccessParams{
