...
func (vcd *TestVCD) Test_CreateLandingZone(check *C) {
	...
	adminVdc, err := adminOrg.GetAdminVDCById(ctx, vcd.nsxtVdc.Vdc.ID, false)
	check.Assert(err, IsNil)

	orgNetwork, err := vcd.nsxtVdc.GetOpenApiOrgVdcNetworkByName(ctx, vcd.config.VCD.Nsxt.RoutedNetwork)
	check.Assert(err, IsNil)
	...
	catalog, err := adminOrg.GetCatalogByName(ctx, vcd.config.VCD.Catalog.NsxtBackedCatalogName, false)
	check.Assert(err, IsNil)
	...
	// Get all
	allEntries, err := vcd.client.GetAllSolutionLandingZones(ctx, nil)
	check.Assert(err, IsNil)
	...
	// Get by ID
	slzById, err := vcd.client.GetSolutionLandingZoneById(ctx, slz.RdeId())
	check.Assert(err, IsNil)
	check.Assert(slzById.RdeId(), Equals, slz.RdeId())

	// Get exactly one
	slzSingle, err := vcd.client.GetExactlyOneSolutionLandingZone(ctx)
	check.Assert(err, IsNil)
	check.Assert(slzSingle.RdeId(), Equals, slz.RdeId())

	// Update
	// Lookup one more Org network and add it
	orgNetwork2, err := vcd.nsxtVdc.GetOpenApiOrgVdcNetworkByName(ctx, vcd.config.VCD.Nsxt.IsolatedNetwork)
	check.Assert(err, IsNil)
	...
	updatedSlz, err := slz.Update(ctx, slzCfg)
	check.Assert(err, IsNil)
	...
	slzByIdErr, err := vcd.client.GetSolutionLandingZoneById(ctx, slz.RdeId())
