...
func test_NsxtDistributedFirewallRule(vcd *TestVCD, check *C, vdcGroupId string, vcdClient *VCDClient, vdc *Vdc, deleteDefaultFirewallRule bool) {
	...
	if deleteDefaultFirewallRule {
		err = vdcGroup.DeleteAllDistributedFirewallRules(ctx)
		check.Assert(err, IsNil)
	}
...
func test_NsxtDistributedFirewallRuleAboveDefault(vcd *TestVCD, check *C, vdcGroupId string, vcdClient *VCDClient, vdc *Vdc) {
	...
	defaultRule, err := vdcGroup.GetDistributedFirewallRuleByName(ctx, defaultRuleName)
	check.Assert(err, IsNil)
	check.Assert(defaultRule, NotNil)

	_, rule1, err := vdcGroup.CreateDistributedFirewallRule(ctx, defaultRule.Rule.ID, randomizedFwRuleDefs[0])
	check.Assert(err, IsNil)

	_, rule2, err := vdcGroup.CreateDistributedFirewallRule(ctx, defaultRule.Rule.ID, randomizedFwRuleDefs[1])
	check.Assert(err, IsNil)
	...
	allRules, err := vdcGroup.GetDistributedFirewall(ctx)
	check.Assert(err, IsNil)
...
func testDistributedFirewallRuleSequence(check *C, randomizedFwRuleSubSet []*types.DistributedFirewallRule, vdcGroup *VdcGroup, reverseOrder bool) {
	...
	for _, rule := range randomizedFwRuleSubSet {
		...
		completeDfw, singleCreatedFwRule, err := vdcGroup.CreateDistributedFirewallRule(ctx, previousRuleId, rule)
		check.Assert(err, IsNil)
	...
	// Perform Update
	ruleById, err := vdcGroup.GetDistributedFirewallRuleById(ctx, allRules.DistributedFirewallRuleContainer.Values[0].ID)
	check.Assert(err, IsNil)

	updatedRuleName := check.TestName() + "-updated"
	ruleById.Rule.Name = updatedRuleName
	updatedRule, err := ruleById.Update(ctx, ruleById.Rule)
	check.Assert(err, IsNil)
	check.Assert(updatedRule.Rule.Name, Equals, updatedRuleName)

	// Delete
	err = updatedRule.Delete(ctx)
	check.Assert(err, IsNil)

	notFoundById, err := vdcGroup.GetDistributedFirewallRuleById(ctx, updatedRule.Rule.ID)
