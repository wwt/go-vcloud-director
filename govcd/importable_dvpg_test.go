...
func (vcd *TestVCD) Test_VcenterImportableDvpg(check *C) {
	...
	// Get all DVPGs
	dvpgs, err := vcd.client.GetAllVcenterImportableDvpgs(ctx, nil)
	check.Assert(err, IsNil)
	check.Assert(len(dvpgs) > 0, Equals, true)

	// Get DVPG by name
	dvpgByName, err := vcd.client.GetVcenterImportableDvpgByName(ctx, vcd.config.VCD.Nsxt.NsxtDvpg)
	check.Assert(err, IsNil)
	...
	allDvpgsWithingVdc, err := nsxtVdc.GetAllVcenterImportableDvpgs(ctx, nil)
	check.Assert(err, IsNil)
	check.Assert(len(allDvpgsWithingVdc) > 0, Equals, true)

	// Get DVPG by name within NSX-T VDC
	dvpgByNameWithinVdc, err := nsxtVdc.GetVcenterImportableDvpgByName(ctx, vcd.config.VCD.Nsxt.NsxtDvpg)
	check.Assert(err, IsNil)
