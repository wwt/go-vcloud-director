...
func (vcd *TestVCD) Test_UIPlugin(check *C) {
	...
	newUIPlugin, err := vcd.client.AddUIPlugin(ctx, vcd.config.Media.UiPluginPath, true)
	check.Assert(err, IsNil)
	...
	_, err = vcd.client.AddUIPlugin(ctx, vcd.config.Media.UiPluginPath, true)
	...
	newUIPlugin, err = vcd.client.GetUIPluginById(ctx, newUIPlugin.UIPluginMetadata.ID)
	check.Assert(err, IsNil)
	...
	_, err = vcd.client.GetUIPluginById(ctx, "urn:vcloud:uiPlugin:00000000-0000-0000-0000-000000000000")
	check.Assert(err, NotNil)
	...
	allUIPlugins, err := vcd.client.GetAllUIPlugins(ctx)
	check.Assert(err, IsNil)
	...
	retrievedUIPlugin, err := vcd.client.GetUIPlugin(ctx, newUIPlugin.UIPluginMetadata.Vendor, newUIPlugin.UIPluginMetadata.PluginName, newUIPlugin.UIPluginMetadata.Version)
	check.Assert(err, IsNil)
	...
	err = newUIPlugin.PublishAll(ctx)
	check.Assert(err, IsNil)
	...
	orgRefs, err := newUIPlugin.GetPublishedTenants(ctx)
	check.Assert(err, IsNil)
	...
	err = newUIPlugin.UnpublishAll(ctx)
	check.Assert(err, IsNil)
	...
	if len(vcd.config.Tenants) > 0 {
		...
		err = newUIPlugin.Unpublish(ctx, existingOrgRefs)
		check.Assert(err, IsNil)
	...
	err = newUIPlugin.Update(ctx, false, false, false)
	check.Assert(err, IsNil)
	...
	err = newUIPlugin.Update(ctx, true, true, true)
	check.Assert(err, IsNil)
	check.Assert(newUIPlugin.UIPluginMetadata.Enabled, Equals, true)
	...
	err = newUIPlugin.Delete(ctx)
	check.Assert(err, IsNil)
	check.Assert(*newUIPlugin.UIPluginMetadata, DeepEquals, types.UIPluginMetadata{})

	// Check that the plugin was correctly deleted
	_, err = vcd.client.GetUIPlugin(ctx, retrievedUIPlugin.UIPluginMetadata.Vendor, retrievedUIPlugin.UIPluginMetadata.PluginName, retrievedUIPlugin.UIPluginMetadata.Version)
	check.Assert(err, NotNil)
	check.Assert(true, Equals, strings.Contains(err.Error(), ErrorEntityNotFound.Error()))
	_, err = vcd.client.GetUIPluginById(ctx, retrievedUIPlugin.UIPluginMetadata.ID)
	check.Assert(err, NotNil)
	check.Assert(true, Equals, strings.Contains(err.Error(), ErrorEntityNotFound.Error()))
}
