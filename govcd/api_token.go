...
func (vcdClient *VCDClient) CreateToken(ctx context.Context, org, tokenName string) (*Token, error) {
	apiTokenParams := &types.ApiTokenParams{
		ClientName: tokenName,
	}

	newTokenParams, err := vcdClient.RegisterToken(ctx, org, apiTokenParams)
	if err != nil {
	...
	token, err := vcdClient.GetTokenById(ctx, tokenUrn)
	if err != nil {
...
func (vcdClient *VCDClient) GetTokenById(ctx context.Context, tokenId string) (*Token, error) {
	...
	err = client.OpenApiGetItem(ctx, apiVersion, urlRef, nil, apiToken.Token, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to get token: %s", err)
	}
...
func (vcdClient *VCDClient) GetAllTokens(ctx context.Context, queryParameters url.Values) ([]*Token, error) {
	client := vcdClient.Client
...
func (vcdClient *VCDClient) GetTokenByNameAndUsername(ctx context.Context, tokenName, userName string) (*Token, error) {
	queryParameters := url.Values{}
	queryParameters.Add("filter", fmt.Sprintf("(name==%s;owner.name==%s;(type==PROXY,type==REFRESH))", tokenName, userName))

	tokens, err := vcdClient.GetAllTokens(ctx, queryParameters)
	if err != nil {
...
func (vcdClient *VCDClient) RegisterToken(ctx context.Context, org string, tokenParams *types.ApiTokenParams) (*types.ApiTokenParams, error) {
	client := vcdClient.Client
	...
	// API version
	err = client.OpenApiPostItemSync(ctx, "", urlRef, nil, tokenParams, newTokenParams)
	if err != nil {
...
// using SetApiToken
func (vcdClient *VCDClient) SetApiTokenFromFile(ctx context.Context, org, apiTokenFile string) (*types.ApiTokenRefresh, error) {
	apiToken, err := GetTokenFromFile(apiTokenFile)
	if err != nil {
		return nil, err
	}

	return vcdClient.SetApiToken(ctx, org, apiToken.RefreshToken)
}
