...
func (vcd *TestVCD) Test_Roles(check *C) {
	...
	allExistingRoles, err := adminOrg.GetAllRoles(ctx, nil)
	check.Assert(err, IsNil)
	...
	for _, oneRole := range allExistingRoles {
		...
		expectOneRoleResultById, err := adminOrg.GetAllRoles(ctx, queryParams)
		check.Assert(err, IsNil)
		check.Assert(len(expectOneRoleResultById) == 1, Equals, true)

		// Step 2.2 - retrieve specific role by using endpoint
		exactItem, err := adminOrg.GetRoleById(ctx, oneRole.Role.ID)
		check.Assert(err, IsNil)
	...
	deletedRole, err := adminOrg.GetRoleById(ctx, createdRole.Role.ID)
