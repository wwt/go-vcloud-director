...
func (vcd *TestVCD) Test_TenantContext(check *C) {
	...
	adminOrgById, err := vcd.client.GetAdminOrgById(ctx, adminOrg.AdminOrg.ID)
	check.Assert(err, IsNil)
	...
	adminOrgByNameOrId, err := vcd.client.GetAdminOrgByNameOrId(ctx, adminOrg.AdminOrg.ID)
	check.Assert(err, IsNil)
	...
	orgById, err := vcd.client.GetOrgById(ctx, org.Org.ID)
	check.Assert(err, IsNil)
	...
	orgByNameOrId, err := vcd.client.GetOrgByNameOrId(ctx, org.Org.ID)
	check.Assert(err, IsNil)
	...
	adminVdcById, err := adminOrg.GetAdminVDCById(ctx, adminVdc.AdminVdc.ID, false)
	check.Assert(err, IsNil)
	...
	adminVdcByNameOrId, err := adminOrg.GetAdminVDCByNameOrId(ctx, adminVdc.AdminVdc.ID, false)
	check.Assert(err, IsNil)
	...
	vdcById, err := adminOrg.GetVDCById(ctx, vdc.Vdc.ID, false)
	check.Assert(err, IsNil)
	...
	vdcByNameOrId, err := adminOrg.GetVDCByNameOrId(ctx, vdc.Vdc.ID, false)
	check.Assert(err, IsNil)
	...
	adminCatalogById, err := adminOrg.GetAdminCatalogById(ctx, adminCatalog.AdminCatalog.ID, false)
	check.Assert(err, IsNil)
	...
	adminCatalogByNameOrId, err := adminOrg.GetAdminCatalogByNameOrId(ctx, adminCatalog.AdminCatalog.ID, false)
	check.Assert(err, IsNil)
	...
	catalogById, err := adminOrg.GetCatalogById(ctx, catalog.Catalog.ID, false)
	check.Assert(err, IsNil)
	...
	catalogByNameOrId, err := adminOrg.GetCatalogByNameOrId(ctx, catalog.Catalog.ID, false)
	check.Assert(err, IsNil)
	...
	if len(vappList) > 0 {
		...
		vapp, err := vdc.GetVAppByName(ctx, vappList[0].Name, false)
		check.Assert(err, IsNil)
		check.Assert(vapp, NotNil)
		vappTenantContext, err := vapp.getTenantContext(ctx)
		check.Assert(err, IsNil)
		checkTenantContext(check, "vapp by name", vappTenantContext, adminOrgTenantContext)

		vappById, err := vdc.GetVAppById(ctx, vapp.VApp.ID, false)
		check.Assert(err, IsNil)
		check.Assert(vappById, NotNil)
		vappTenantContext, err = vappById.getTenantContext(ctx)
		check.Assert(err, IsNil)
		checkTenantContext(check, "vapp by ID", vappTenantContext, adminOrgTenantContext)

		vappByNameOrId, err := vdc.GetVAppByNameOrId(ctx, vapp.VApp.ID, false)
		check.Assert(err, IsNil)
		check.Assert(vappByNameOrId, NotNil)
		vappTenantContext, err = vappByNameOrId.getTenantContext(ctx)
		check.Assert(err, IsNil)
		...
		if len(vmList) > 0 {
			...
			vm, err := vcd.client.Client.GetVMByHref(ctx, vmList[0].HREF)
			check.Assert(err, IsNil)
			check.Assert(vm, NotNil)

			vmTenantContext, err := vm.getTenantContext(ctx)
			check.Assert(err, IsNil)
